...
user_channels = {}  # {user_id: [channel_data]}
...
def channels_keyboard(user_id):
    """Клавиатура со списком каналов пользователя"""
    builder = InlineKeyboardBuilder()
...
async def show_settings(callback: types.CallbackQuery):
    ...
    if not channels:
        await callback.message.edit_text(
            "❌ У тебя нет каналов. Создай канал через /start.",
            reply_markup=main_keyboard()
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        "📢 <b>Выбери канал для настройки:</b>",
        reply_markup=channels_keyboard(user_id)
    )
    await callback.answer()
...
async def start_rename(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "📝 Введи новое название для канала:",
        reply_markup=None
    )
...
async def start_bio(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(channel_id=channel_id)
    await callback.message.edit_text(
        "📄 Введи новое описание для канала:",
        reply_markup=None
    )
...
async def process_bio(message: types.Message, state: FSMContext):
    ...
    try:
        await message.bot.set_chat_description(
            chat_id=channel_id,
            description=new_bio
        )
...
async def start_avatar(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(channel_id=channel_id)
    await callback.message.edit_text(
        "🖼️ Отправь мне новую аватарку для канала.\n"
        "Фото должно быть квадратным.",
        reply_markup=None
    )
...
async def process_transfer_owner(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Снимаем права с текущего владельца
        await message.bot.promote_chat_member(
            chat_id=chat.id,
            user_id=message.from_user.id,
